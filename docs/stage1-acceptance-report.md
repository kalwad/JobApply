...
- **History note:** older commits on this branch still contain the digit string `2488322855` inside `extension/tests/content.test.js` (introduced in phone-flood fix era). Current tip uses `5551234567`. History rewrite (`git filter-repo` / BFG) requires explicit owner approval — do not force-push `main`.
